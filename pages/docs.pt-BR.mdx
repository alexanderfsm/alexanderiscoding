import projects from '../lib/projects.json';
import services from '../lib/services.json';
import Box from '../components/box';
import Link from 'next/link'

export default function Page() {
  return (
    <main className="relative">
      <div className="mx-auto">
        <div className="py-16 lg:text-center">
          <p className="text-base font-semibold leading-6 tracking-wide text-blue-600 uppercase dark:text-gray-400">
            Projetos
          </p>
          <h1 className="mt-2 text-3xl font-extrabold leading-8 tracking-tight text-gray-900 md:text-5xl dark:text-white sm:text-4xl sm:leading-10">
            Quais estão Disponíveis?
          </h1>
          <p className="max-w-3xl mt-4 text-xl leading-7 text-gray-500 dark:text-gray-400 lg:mx-auto">
            Os projetos foram pensados para ajudar no desenvolvimento de novas ideias, são ferramentas
            de código aberto, que podem se integrar entre si, onde você próprio hospeda e guarda as
            informações geradas por eles, são projetos de utilidade pública onde outros podem ser pagos ou as vezes díficil de usar.
          </p>
        </div>
      </div>

      <div className="grid mt-12 gap-x-6 gap-y-12 sm:grid-cols-2 lg:mt-16 lg:gap-x-8 lg:gap-y-12">
        {projects ?
          projects.map((project, index) => {
            return <Box {...project} key={index} type="project" />
          })
          :
          null
        }
      </div>

      <div className="mx-auto">
        <div className="py-16 lg:text-center">
          <p className="text-base font-semibold leading-6 tracking-wide text-blue-600 uppercase dark:text-gray-400">
            Serviços
          </p>
          <h1 className="mt-2 text-3xl font-extrabold leading-8 tracking-tight text-gray-900 md:text-5xl dark:text-white sm:text-4xl sm:leading-10">
            Quais estão Disponíveis?
          </h1>
          <p className="max-w-3xl mt-4 text-xl leading-7 text-gray-500 dark:text-gray-400 lg:mx-auto">
            Os serviços foram pensados para suprir algo que pode existir, mas não fornece
            de forma organizada o conteúdo, ou pode ter os conteúdos, mas não fornece
            um meio de utilizar esse conteúdo em outros lugares, gostariamos de trazer serviços
            que podem ser precários para os usuarios, ou um meio de ter novos panoramas.
          </p>
        </div>
      </div>

      <div className="grid mt-12 gap-x-6 gap-y-12 sm:grid-cols-2 lg:mt-16 lg:gap-x-8 lg:gap-y-12">
        {services ?
          services.map((project, index) => {
            return <Box {...project} key={index} type="service" />
          })
          :
          null
        }
      </div>

      <div className="mx-auto max-w-7xl sm:px-6 lg:px-8">
        <div className="max-w-xl pt-20 pb-24 mx-auto space-y-6 text-center">
          <div className="mt-2 text-2xl font-extrabold leading-8 tracking-tight text-gray-900 dark:text-white sm:text-4xl sm:leading-10">
            Precisando de outro projeto ou serviço?
          </div>
          <div className="mx-auto rounded-md">
            <Link
              href="mailto:github@alexanderiscoding.com"
              className="inline-flex items-center justify-center w-auto px-8 py-3 text-base font-medium text-white no-underline bg-black border border-transparent rounded-md dark:bg-white dark:text-black betterhover:dark:hover:bg-gray-300 betterhover:hover:bg-gray-700 md:py-3 md:text-lg md:px-10 md:leading-6">
              Envie uma Ideia
            </Link>
          </div>
        </div>
      </div>
    </main>

);
}
